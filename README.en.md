...
> The whole hub is generated by Python (stdlib only). Usage and how to extend in [`scripts/README.md`](scripts/README.md).

| Script | Type | What it does |
|---|---|---|
| [`scripts/maestri_build.py`](scripts/maestri_build.py) | library | `Partitura` class, `.maestripartitura` serialization, ropePoints, layout, portals. |
| [`scripts/roles_lib.py`](scripts/roles_lib.py) | library | Responsibility prompts (pt-BR) and note templates. |
| [`scripts/generate_partituras.py`](scripts/generate_partituras.py) | generator | Builds the 257 scores by area, the bundles, and the catalogs. |
| [`scripts/generate_hub.py`](scripts/generate_hub.py) | generator | Generates `roles/` (role.json), `notas/`, and `instrucoes/`. |
| [`tests/validate_partituras.py`](tests/validate_partituras.py) | validation | Compares each score with the official reference (zero mismatches). |
| [`tests/validate_hub.py`](tests/validate_hub.py) | validation | Validates the role.json files, notes, and CLAUDE.md/AGENTS.md pairs. |
